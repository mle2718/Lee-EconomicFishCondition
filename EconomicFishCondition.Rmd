...
Table \ref{priceconditionreg} reports the results of regression price on condition at the annual level. There are up to 26 data points (years) per species.  That table is intended to give us a quick look at species where we might pick up an effect of condition on price.  Of course, there are major OVs (landed quantities and import prices) here so don't over-interpret.
...
```
  ...
  \begin{table}
    \begin{center}
        \begin{tabular}{lrrrr}
          \input{./tables/common/price_condition_reg.tex}
        \end{tabular}  
          \caption{Coefficients for an OLS regression of  price on condition. Sorted High to low based on the condition coefficient. \label{priceconditionreg}}
...
silver_hake_01.do - this is my first set of exploratory regressions. The code runs quickly (OLS and IVs).  These models not good models, but we can still learn something from them.  
silver_hake_02.do - this is my second set of exploratory regressions. The code runs slower because I used lots of Fixed Effects for permits and sometimes for dealers and fitted the models with REGHDFE and IVREGHDFE.   Some of these *might* be candidates for an acceptable specification.
silver_hake_03.do - this is my third set of exploratory regressions.  Some of these *might* be candidates for an acceptable specification. I experimented with 
silver_hake04_import_pounds.do - this is my fourth set of exploratory regressions.  Some of these *might* be candidates for an acceptable specification. I experimented with 

## To do
You need to go back and manually code the market shift *nespp4 interactions. letting ivreghdfe do it by itself makes things hard to interpret.  flip it back to i0.mkt_shift

depvars ib5090.nespp4 ib5090.nespp4#i0.mkt_shift 
or
depvars ib5090.nespp4 ib5090.nespp4#i1.mkt_shift 

This is the same. I find the 2nd more intuitive to interpret, but they do the same thing.  Also need to have the interactions sent to a table (they are not currently).
...
\newpage
  \begin{table}
    \begin{center}
        {\scriptsize
          \input{./tables/silverhake/silver_hake_years.tex}
...
\newpage
  \begin{table}
    \begin{center}
      {\scriptsize
        \input{./tables/silverhake/silver_hake_month_week.tex}
...
\begin{landscape}
  \begin{table}
    \begin{center}
        {\scriptsize
          \input{./tables/silverhake/silver_hake3Q.tex}
...
\newpage
  \begin{table}
    \begin{center}
        {\scriptsize
          \input{./tables/silverhake/silver_hake_condition4Q.tex}
...
\newpage
  \begin{table}
    \begin{center}
        {\scriptsize
          \input{./tables/silverhake/silver_hake_ihs.tex}
...
\newpage
  \begin{table}
    \begin{center}
        {\scriptsize
          \input{./tables/silverhake/silver_hake_ihs4Q.tex}
