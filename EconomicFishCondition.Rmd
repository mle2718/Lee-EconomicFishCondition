...
Day-level heterogeneity is most likely related to quantity supplied.  We should either include day-level fixed effects or quantity supplied.  Daily quantity supplied is likely to be endogenous; I find two arguements compelling. The first, there are demand characteristis unobserved to us that are observed by sellers. This makes the error term in our regression correlated with quantity supplied.  Second, analgous to @Graddy2010; , sellers (fishing vessels) have an ability to inventory a bit when there are low prices and deplete that inventory when there are high prices. This happens, even with a highly perishable good because people can adjust the timing of when they land fish at the margin.
...
We can put "lot size" in as an explanatory variable. I'm expecting this coefficient to be non-negative.  A negative coefficient would mean that buyers could split large lots and get a higher price per pound. This would be "wierd" and inconsistent with increasing returns to scale in the processing sector. Processing and wholesaling fish entails some quasi-fixed costs.  A small lot is going to be worth less than a large lot. There may be some confounding due to the "aggregate supply effect"
# Data overview
...
I think this is coded properly in 02 and needs to be propagated to 3 4importpounds
...
1. Daily landings effect is fairly stable -0.5 to -0.7.  This is reasonable.
1. Import price effect is 0.5 to 0.77.  This is reasonable and consistent with imports as substitutes. IV49, which uses import quantities has a negative sign here, consistent with imports being substitutes.
1. The income effect is -1.1. This means that if incomes go up by 1\%, prices go down by just over 1\%. This doesn't make much sense.  I suspect this is correlated with something else.  Not sure what.
